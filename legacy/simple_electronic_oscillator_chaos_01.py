#!/usr/bin/env python3
"""
Bifurcation diagram for the "simple chaotic oscillator"
(Tamaševičius et al., Eur. J. Phys. 26 (2005) 61–63).

Implements the nondimensional ODEs (eq. 4 in the paper):
    xdot = y
    ydot = a*y - x - z
    eps*zdot = b + y - c*(exp(z) - 1)

Produces a bifurcation diagram: local maxima of x vs control parameter a (like Fig. 2).

Key changes vs the earlier version:
- Uses a single solve_ivp call per 'a' (uniform t_eval), then discards transient samples.
- Uses BDF by default (often faster than Radau here).
- Adds progress printing and periodic autosave of partial results.
- Uses a faster “first-pass” scan configuration (fewer a points, shorter integration).
- Slightly looser tolerances for speed (still fine for bifurcation structure).
- Sets max_step to stabilize runtimes.

If you want to map from component values:
    rho = sqrt(L/C)
    tau = sqrt(L*C)
    eps = Cstar / C
    a = (k - 1) * R / rho
    b = rho*I0 / VT  where I0 ~ Vb/R0 and VT ~ 25.85 mV
    c = rho*Is / VT  (Is diode saturation current; uncertain, so often treated as small constant)
...
def rhs(theta: float, state: np.ndarray, p: ModelParams) -> np.ndarray:
    ...
    # Clip z to avoid exp overflow; tighter than before for speed/stability
...
# -----------------------------
# Peak picking and bifurcation scan
# -----------------------------
...
def simulate_and_collect_maxima(
    ...
    Discard early samples (transient), then extract local maxima of x.
    """
...
def bifurcation_scan(
    ...
    Uses continuation: last state from a short integration at each 'a'
    seeds the next value of 'a'. Also prints progress and autosaves.
    """
    ...
    for i, a in enumerate(a_values):
        ...
        if i % 10 == 0:
            ...
            print(f"[{i:4d}/{len(a_values)}] a={a:.5f} maxima={maxima.size:3d} "
                  f"points={len(A_list):7d} elapsed={elapsed:.1f}s", flush=True)
        ...
        if autosave_every > 0 and (i % autosave_every == 0) and len(A_list) > 0:
            ...
            # no print needed; but you can uncomment:
            # print(f"[SAVE] wrote {autosave_path} with {arr.shape[0]} rows", flush=True)

        # Continuation seed update: short integration (cheap)
        sol_seed = solve_ivp(
            fun=lambda t, s: rhs(t, s, p),
            t_span=(0.0, 40.0),
            y0=y0,
...
# ============================================================
# Feigenbaum-zoom add-on (drop this into your script)
# ============================================================
# Goal:
# 1) Take your coarse “split indices” (e.g. 28, 43, 48) from the coarse scan.
# 2) Convert them into approximate a-values using the SAME a-grid as the coarse scan.
# 3) Predict where the next split should lie if Feigenbaum scaling is kicking in.
# 4) Do a dense scan in a narrow window around that prediction.
# 5) Automatically locate the next splitting a4 by detecting when the number of
#    distinct maxima doubles (period 2^n -> 2^(n+1)) and using bisection.
#
# It reuses rhs(), ModelParams, and simulate_and_collect_maxima() from your script.
...
def estimate_next_index_by_feigenbaum(n1: float, n2: float, n3: float, delta_feig: float = 4.669201609102990) -> float:
    """
...
def _cluster_count(values: np.ndarray, tol: float = 1e-3) -> int:
    ...
    For period-doubling using maxima, this approximates the orbit period:
      period-1 -> 1 cluster, period-2 -> 2 clusters, etc.
    """
    if values.size == 0:
        return 0
...
def period_from_maxima(maxima: np.ndarray, tol: float = 1e-3, max_reasonable: int = 64) -> int:
    """
    Convert maxima array into an estimated period = number of distinct maxima values.
    Caps at max_reasonable: if it exceeds this, treat as 'chaotic/large' period.
    """
...
def period_at_a(
    ...
    """Integrate at a, return estimated period (1,2,4,8,...) based on clustered maxima."""
    p = ModelParams(a=float(a), b=float(b), c=float(c), eps=float(eps))
    maxima = simulate_and_collect_maxima(
        p=p,
        theta_transient=theta_transient,
        ...
        dt_sample=dt_sample,
        y0=seed_state,
        # these match the faster settings in the updated script
        solver="BDF",
        rtol=1e-6,
        atol=1e-9,
        max_step=2.0,
        max_maxima=120,   # a bit more than before helps period detection
    )
...
def find_period_doubling_a(
    ...
    """
    Locate the parameter value a* where the orbit transitions from target_left_period
    to approximately 2*target_left_period, using bisection.

    Preconditions (we’ll check and try to fix by nudging if needed):
      period(a_lo) ~ target_left_period
      period(a_hi) >= 2*target_left_period   (or already beyond)
    """
    ...
    # Try to enforce a bracket; if not, just proceed but warn.
    if p_lo != target_left_period:
        print(f"[WARN] Left period {p_lo} != expected {target_left_period}. "
              f"Try increasing theta_transient/theta_sample or adjusting tol_max_cluster.", flush=True)

    if p_hi < 2 * target_left_period:
        print(f"[WARN] Right period {p_hi} < expected doubling {2*target_left_period}. "
              f"Widen the bracket window.", flush=True)
    ...
    for it in range(max_iter):
        ...
        # We want the boundary where it stops being target_left_period and becomes larger
...
def feigenbaum_zoom_from_indices(
    ...
    eps: float = 0.13,
    c: float = 4e-9,
    # integration params (use stronger settings than the coarse scan for period detection)
    theta_transient: float = 1800.0,
    theta_sample: float = 1800.0,
    dt_sample: float = 0.20,
    seed_state: np.ndarray | None = None,
    # maxima clustering tolerance
    tol_max_cluster: float = 2e-3,
    # how wide to scan around predicted a4
    window_halfwidth_in_index_units: float = 2.0,
) -> None:
    """
    End-to-end helper:
    - Convert coarse indices to a1,a2,a3
    - Predict next split location via Feigenbaum scaling
    - Bracket around predicted a4 and bisection-search it
    - Print updated delta estimates in BOTH index-units and a-units
    """
    if seed_state is None:
        seed_state = np.array([0.1, 0.0, 0.0], dtype=float)
    ...
    # Build a bracket window around predicted n4
    ...
    # Determine the "left period" we expect before the next doubling.
    # With three split points, you’re likely at period 8 -> 16 next, but let's infer from a just below a3.
    # We'll sample at slightly below a3 to estimate current period.
    a_test_left = max(a_min, a3 - 3*da)
    p_left = period_at_a(
        a_test_left, b, eps, c, theta_transient, theta_sample, dt_sample, seed_state, tol_max_cluster
    )
    # Round p_left to nearest power of 2 for robustness
    if p_left < 1:
        p_left = 1
    pow2 = 1
    ...
    # Convert a4 back to “index units” for comparison and compute updated delta estimate
    n4 = (a4 - a_min) / da
    d3 = n4 - n3
    delta_est_4 = d2 / d3
    ...
    print("Tip: If this is still far off, increase theta_transient/theta_sample and/or tighten the bracket window.")
    print("===============================\n")


# ------------------------------------------------------------
# HOW TO USE IT (paste into your main() after the coarse scan)
# ------------------------------------------------------------
# In your current script, you likely have:
#   a_values = np.linspace(0.0, 0.85, 220)
# so set:
#
...
#
# That will:
# - print a1,a2,a3 from your indices
# - predict where the next split should be
# - bisection-search it
# - print an improved delta estimate
...
def main():
    ...
    # FAST FIRST-PASS SCAN SETTINGS (recommended):
    a_values = np.linspace(0.0, 0.85, 220)
    ...
    # Save final data
    if A.size > 0:
        np.savetxt("bifurcation.csv",
                   np.column_stack([A, X]),
                   delimiter=",", header="a,x_max", comments="")
    ...
    plt.title("Bifurcation diagram: maxima of x vs a (eq. 4)")
    plt.tight_layout()

    # Prefer saving as well as showing (Spyder backend quirks)
    plt.savefig("bifurcation.png", dpi=200)
    print("[DONE] wrote bifurcation.png", flush=True)
    plt.show()

    # Optional: show how to compute a from the paper’s component values
    circ = CircuitParams(R2=4e3)  # change R2 to explore mapping to 'a'
